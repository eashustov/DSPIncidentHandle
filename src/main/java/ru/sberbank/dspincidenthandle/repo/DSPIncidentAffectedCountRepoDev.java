...
public interface DSPIncidentAffectedCountRepoDev extends DSPIncidentAffectedCountRepo{
    ...
    @Query(value = "select p.HPC_AFFECTED_ITEM_NAME as Affected_Item, COUNT (p.NUMBER) AS count_Inc from SMPRIMARYSAFE p GROUP BY Affected_Item ORDER BY count_Inc DESC", nativeQuery = true)
