...
//    List<IUspIncidentDataCountPerMonth> findIncCountPerMonth(@Param("startDate") String startDate, @Param("endDate") String endDate, @Param("groupAssignment") String groupAssignment);
    ...
    @Query(value = "select p.HPC_AFFECTED_ITEM_NAME as Affected_Item, MONTHNAME(PARSEDATETIME(p.PLAN_OPEN, 'yyyy.MM.dd hh:mm')) AS month,  MONTH(PARSEDATETIME(p.PLAN_OPEN, 'yyyy.MM.dd hh:mm')) AS month_number, YEAR(PARSEDATETIME(p.PLAN_OPEN, 'yyyy.MM.dd hh:mm')) AS year, COUNT (p.NUMBER) AS count_inc from SMPRIMARY p \n" +
            "GROUP BY Affected_Item, MONTHNAME(PARSEDATETIME(p.PLAN_OPEN, 'yyyy.MM.dd hh:mm')), MONTH(PARSEDATETIME(p.PLAN_OPEN, 'yyyy.MM.dd hh:mm')), YEAR(PARSEDATETIME(p.PLAN_OPEN, 'yyyy.MM.dd hh:mm'))\n" +
            "ORDER BY Affected_Item, year, month_number ASC", nativeQuery = true)
    List<IDSPIncidentDataCountPerMonth> findIncAffectedItemCountPerMonth();
